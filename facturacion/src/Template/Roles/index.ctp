                ...
                <tbody>
                   <?php foreach ($roles as $rol): ?>
                    <tr>
                        ...
                        <td><?= h($rol->name) ?></td>
                        <td><?= $this->Number->format($rol->status) ?></td>
                        <td><?= h($rol->created) ?></td>
                        <td><?= h($rol->modified) ?></td>
                        <td class="actions">
                            <?= $this->Html->link(__('View'), ['action' => 'view', $rol->id],['class'=>'btn btn-sm btn-info']  ) ?>
                            <?= $this->Html->link(__('Edit'), ['action' => 'edit', $rol->id], ['class' =>'btn btn-sm btn-warning']  ) ?>
                            <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $rol->id],
                            ['confirm' => __('Are you sure you want to delete # {0}?', $rol->id), 'class'=>'btn btn-sm btn-danger']) ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
